...
public class DirectAction extends ERD2WDirectAction {
    ...
    private boolean isAuthorized() {
        ...
        try {
            ...
            // TODO: replace with a real user lookup
            return "tedpet".equals(user) && "3368".equals(pass);
...
//                                 "personSecurity": null | { "id": N } }
 public WOActionResults createPersonAction() {
     ...
     try {
         ...
         if (root.has("password"))      p.setPassword(root.path("password").asText(null));
 ...
 public WOActionResults updatePersonAction() {
     ...
     try {
         ...
         // Only change password when the client sends a non-null, non-empty value.
         JsonNode pw = root.path("password");
         if (!pw.isMissingNode() && !pw.isNull()) {
             String s = pw.asText("");
             if (!s.isEmpty()) p.setPassword(s);
         }

//         // Only swap PersonSecurity when client explicitly picks one (don't null it out).
//         if (root.has("personSecurity") && !root.path("personSecurity").isNull()) {
//             PersonSecurity ps = resolveSecurity(ec, root.path("personSecurity"));
//             if (ps != null) p.setPersonSecurity(ps);
//         }
         JsonNode psNode = root.path("personSecurity");
         if (!psNode.isMissingNode() && !psNode.isNull()) {
             // Defensive — create one if a legacy Person somehow has no security row.
             if (p.personSecurity() == null) {
                 p.setPersonSecurity((PersonSecurity)
                     EOUtilities.createAndInsertInstance(ec, "PersonSecurity"));
             }
             applySecurity(p.personSecurity(), psNode);   // writes the six booleans
         }
