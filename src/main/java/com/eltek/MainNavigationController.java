	...
	public WOComponent listPersonAction() {
		...
		try {
			...
			ERXFetchSpecification<Person> fs = 
					new ERXFetchSpecification<Person>(Person.ENTITY_NAME, Person.CURRENT.eq(true), null);
	...
	public WOComponent listInstrumentAction() {
		...
		try {
			...
			ERXFetchSpecification<Instrument> fs = 
					new ERXFetchSpecification<Instrument>(Instrument.ENTITY_NAME, Instrument.AVAILABLE.eq(true), null);
